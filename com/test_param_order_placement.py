...
class TestParamOrderPlacement():
    ...
    def test_param_order_placement(self, setup_session,test_input):
        ...
        login_page.login(test_input.get("user_name"), test_input.get("password"))
        if order.add_item_to_cart() is False:
            self.log.error(f"iteration {test_input} failed")
